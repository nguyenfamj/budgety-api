...
public class BudgetController {
        ...
        if (requestBody.getYearMonth() == null) {
            List<Budget> budgetsFromRequest = budgetRepository
                    .findByBudgetKeysCategoryIn(userCategories,
                            Sort.by("budgetKeys.yearMonth").descending().and(Sort.by("amount").descending()));

            return ResponseEntity.ok()
                    .body(new BudgetResponse(HttpServletResponse.SC_OK,
                            "All budgets for user " + authenticatedUser.getUserName() + " retrieved successfully",
                            budgetsFromRequest));
        }
    ...
    public ResponseEntity<?> createBudgetByMonthAndCategory(@Valid @RequestBody CUBudgetRequest requestBody) {
        ...
        Budget foundBudget = budgetRepository.findByBudgetKeysYearMonthAndBudgetKeysCategory(requestBody.getYearMonth(),
                categoryFromRequest);
        ...
        if (Objects.isNull(savedBudget)) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new BudgetResponse(
                    HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Cannot create new budget, please try again"));
        }
        ...
        return ResponseEntity.ok()
                .body(new BudgetResponse(HttpServletResponse.SC_OK, "Budget created successfully", budgetList));
    }
    ...
    public ResponseEntity<?> updateBudget(@PathVariable Long budgetId,
        ...
        return ResponseEntity.ok()
                .body(new BudgetResponse(HttpServletResponse.SC_OK, "Budget updated successfully", budgetList));
    }
    ...
    public ResponseEntity<?> deleteBudgetById(@PathVariable Long budgetId) {
        ...
        return ResponseEntity.ok()
                .body(new BudgetResponse(HttpServletResponse.SC_OK, "Budget " + budgetId + " deleted successfully",
                        budgetList));
    }
}
